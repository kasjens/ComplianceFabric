...
The generation and admission steps are proven by two end-to-end harnesses. `test/e2e/supply-chain.sh` runs the whole chain locally and reproducibly with a key-based cosign: it generates an SBOM with syft, attaches the SBOM and a SLSA provenance attestation and signs the image with cosign, then on a live kind cluster running Kyverno asserts that an unsigned image is denied at admission and the signed image is admitted, and finally turns the verified provenance attestation back into satisfied evidence through this producer. `test/e2e/keyless-ci.sh` (the `keyless-e2e` CI job, `id-token: write`) covers the one strand that cannot run locally: it signs a test image with the GitHub Actions OIDC identity and asserts the same deny/admit behaviour against the production `verify-image-signatures.yaml` policy verbatim. Both kind-based e2e jobs pass in CI. Cosign is pinned to v2 in both harnesses because cosign v3's default new-bundle signature format is unreadable by the cosign library in Kyverno v1.11.5. The dedicated SBOM-content evidence producer described above (`fabric sbom`) now complements the provenance producer.

The generation harness is now bound into the release pipeline itself, not only an e2e proof. `fabric release-gate <manifest-file> [--ledger <path>]` is the release evidence gate: a declarative release manifest (`release/manifest.json`) names each generated supply-chain artifact (the SBOM, the SLSA provenance attestation) and the producer that judges it, and the gate runs every producer once over those files, appends the resulting records to a fresh per-release ledger, verifies the chain, and clears the release (exit 0) only when every control is satisfied — so a banned component, an artifact built outside the trusted pipeline, or a failed evaluation gate blocks the ship (exit 1). The producers are the same shared `internal/collect` registry the CLI and the continuous collector use, so a release-time judgment and an after-the-fact one are identical; the manifest loading and the gate decision are unit-covered in `internal/release`, leaving only file and ledger I/O in the CLI shell. The release workflow (`.github/workflows/release.yml`) runs it on a tagged release: it builds `fabric`, generates a real CycloneDX SBOM with syft, feeds it through `fabric release-gate`, and uploads the verified ledger and the SBOM as release evidence artifacts. The end-to-end proof ([`test/e2e/release.sh`](test/e2e/release.sh), CI job `release-e2e`) generates a genuine syft SBOM, pairs it with a provenance attestation, and asserts the gate clears a clean release (the verified ledger is the evidence artifact) and blocks both a present banned component and an untrusted builder.
