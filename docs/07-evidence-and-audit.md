...
**A limitation worth stating plainly:** `artifact-ref` is only populated when the producer actually has a digest to record. `fabric sbom` takes it from the scanned component's purl, which exists for an **image** scan but not for a **directory** scan — and this project's own release pipeline runs `syft dir:.`, so its SBOM record carries no artifact-ref. The transparency-log pivot for a directory-scanned release therefore comes from the **provenance** record, whose subject digest is the built binary's own hash. If you need the pivot from SBOM content specifically, scan an image rather than a tree. A record with an empty artifact-ref is still valid evidence about content; it simply cannot be pivoted to an artifact by digest.
