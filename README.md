...
- **Control authoring, policy, and enforcement (Phase 1, done).** OSCAL catalogs (EU GMP Annex 11, 21 CFR Part 11), a pharma MES baseline profile, and a native composer (`fabric generate`) that assembles a Kyverno policy set for the selected controls from the policy library, resolving each control's rules to the policy resources that enforce them and carrying `fabric.control-id` traceability (it selects and copies reviewed policy resources; it does not author policy bodies from control text). Admission and runtime enforcement are proven on a live kind cluster in CI.
- **Trusted delivery (Phase 2, done).** SBOM and SLSA provenance evidence producers, Sigstore keyless signature verification at admission, GitOps change-control evidence, and a release evidence gate (`fabric release-gate`) that binds supply-chain evidence generation into the release pipeline. Proven end to end with syft and cosign in CI. The release manifest declares both an SBOM and a SLSA provenance source, so a tagged release is gated on its own supply-chain evidence and the artifacts, the SBOM, the provenance and the evidence ledger are all signed keylessly with the workflow's OIDC identity.
